...
ws)
    ...
    if [ ! -f /tmp/cert.pem ]; then
        openssl req -subj '/CN=www.example.com/O=Internet Corporation for Assigned Names and Numbers/C=US/OU=Technology' -new -x509 -sha256 -days 3650 -nodes -out /tmp/cert.pem -keyout /tmp/key.pem 2>&1 > /dev/null
    fi
...
httpd -config /tmp/config.json 2>&1 > /dev/null
